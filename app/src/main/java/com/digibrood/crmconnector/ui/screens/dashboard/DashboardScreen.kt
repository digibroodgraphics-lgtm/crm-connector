...
private fun DiagnosticsCard(state: DashboardUiState) {
    Card(
        ...
        Column(modifier = Modifier.fillMaxWidth().padding(16.dp)) {
            ...
            DiagRow("Calls captured by app", state.diagCapturedByApp.toString())
            DiagRow("Last call captured", state.diagLastCapturedCall)
            DiagRow("System log visible", state.diagCallsVisible.toString())
            DiagRow("System log after activation", state.diagCallsAfterActivation.toString())
            DiagRow("System log latest (may be stale)", state.diagLatestCall)
